...
class BiospecimenDiagnosisListAPI(CRUDView):
    ...
    def post(self):
        ...
        try:
            bs_gf = (BiospecimenDiagnosisSchema(strict=True)
                     .load(body).data)
        ...
        return BiospecimenDiagnosisSchema(
            201, 'biospecimen_diagnosis {} created'.format(bs_gf.kf_id)
        ).jsonify(bs_gf), 201
...
class BiospecimenDiagnosisAPI(CRUDView):
        ...
        template:
          ...
          properties:
            resource:
              BiospecimenDiagnosis
        """
        bs_gf = BiospecimenDiagnosis.query.get(kf_id)
        if bs_gf is None:
            abort(404, 'could not find {} `{}`'
                  .format('biospecimen_diagnosis', kf_id))

        return BiospecimenDiagnosisSchema().jsonify(bs_gf)
    ...
    def patch(self, kf_id):
        ...
        return BiospecimenDiagnosisSchema(
            200, 'biospecimen_diagnosis {} updated'.format(bs_gf.kf_id)
        ).jsonify(bs_gf), 200
    ...
    def delete(self, kf_id):
        ...
        return BiospecimenDiagnosisSchema(
            200, 'biospecimen_diagnosis {} deleted'.format(bs_gf.kf_id)
        ).jsonify(bs_gf), 200
