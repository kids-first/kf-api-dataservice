...
class SequencingExperimentSchema(BaseSchema):
    sequencing_center_id = field_for(
        SequencingExperiment,
        "sequencing_center_id",
        required=True,
        load_only=True,
    )
    experiment_strategy = field_for(
        SequencingExperiment,
        "experiment_strategy",
        validate=enum_validation_generator(EXPERIMENT_STRATEGY_ENUM),
    )
    platform = field_for(
        SequencingExperiment,
        "platform",
        validate=enum_validation_generator(PLATFORM_ENUM),
    )
    library_strand = field_for(
        SequencingExperiment,
        "library_strand",
        validate=enum_validation_generator(LIBRARY_STRAND_ENUM),
    )
    library_selection = field_for(
        SequencingExperiment,
        "library_selection",
        validate=enum_validation_generator(LIBRARY_SELECTION_ENUM),
    )

    library_prep = field_for(
        SequencingExperiment,
        "library_prep",
        validate=enum_validation_generator(LIBRARY_PREP_ENUM),
    )
    ...
    max_insert_size = field_for(
        SequencingExperiment,
        "max_insert_size",
        validate=validate_positive_number,
    )
    mean_insert_size = field_for(
        SequencingExperiment,
        "mean_insert_size",
        validate=validate_positive_number,
    )
    mean_depth = field_for(
        SequencingExperiment, "mean_depth", validate=validate_positive_number
    )
    total_reads = field_for(
        SequencingExperiment, "total_reads", validate=validate_positive_number
    )
    mean_read_length = field_for(
        SequencingExperiment,
        "mean_read_length",
        validate=validate_positive_number,
    )
    experiment_date = field_for(
        SequencingExperiment, "experiment_date", field_class=DateOrDatetime
    )
    ...
    _links = ma.Hyperlinks(
        {
            ...
            "sequencing_center": ma.URLFor(
                "api.sequencing_centers", kf_id="<sequencing_center_id>"
            ),
            "sequencing_experiment_genomic_files": ma.URLFor(
                "api.sequencing_experiment_genomic_files_list",
                sequencing_experiment_id="<kf_id>",
            ),
            "genomic_files": ma.URLFor(
                "api.genomic_files_list", sequencing_experiment_id="<kf_id>"
            ),
        },
