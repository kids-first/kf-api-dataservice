...
class PhenotypeTest(FlaskTestCase):
    ...
    def test_post_missing_req_params(self):
        """
        Test create phenotype that is missing required parameters in body
        """
        # Create phenotype data
        kwargs = {
            'phenotype': 'Hand Tremor',
        ...
        response = json.loads(response.data.decode("utf-8"))
        # Check error message
        message = 'could not create phenotype'
        self.assertIn(message, response['_status']['message'])
        # Check field values
        ph = Phenotype.query.first()
    ...
    def test_patch_bad_input(self):
        """
        Test update existing phenotype with bad input
        """
        # Create and save phenotype to db
        kwargs = self._create_save_to_db()
        ...
        self.assertEqual(p.phenotype, kwargs.get('phenotype'))
        self.assertEqual(p.age_at_event_days, kwargs.get('age_at_event_days'))
