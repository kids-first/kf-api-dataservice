...
class ModelTest(FlaskTestCase):
	...
	def test_create(self):
		...
		self.assertEqual(new_phenotype.external_id, data['external_id'])
	...
	def test_create_via_participant(self):
		...
		for p in Participant.query.first().phenotypes:
			self.assertIn(p.phenotypes, pheno)
	...
	def test_find_phenotype(self):
		...
		self.assertEqual(ph.phenotype, phe[0])
	...
	def test_update_phenotype(self):
		...
		# Update and save
		phe = Phenotype.query.filter_by(phenotype=ph[0]).one_or_none()
